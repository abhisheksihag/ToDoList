...
function renderTodoList(filteredTodos = todoList) {
  ...
  filteredTodos.forEach((todo, index) => {
        ...
        listItem.innerHTML = `
          ...
          <span>Due Date:
          <input type="date" id="dueDateInput${index}" value="${todo.dueDate}" ${!todo.editing ? 'disabled' : ''}>
        </span>
        <span>Priority:
          ...
          <span class="tags">Tags: ${todo.tags || ''}</span> <!-- Display the tags here -->
  ...
  todoList.forEach(todo => {
    if (todo.reminder && todo.rem_stat==false) {
      ...
      if (reminderDate <= currentDate) {
        ...
        // Render the updated todo list to reflect the triggered reminder status
        renderTodoList();
      }
    }
  });
